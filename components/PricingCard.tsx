...
const PricingCard: React.FC<PricingCardProps> = ({
  ...
  const formatPrice = () => {
    ...
    return `${plan.price.toFixed(2)}€/${plan.interval === 'monthly' ? 'mois' : 'an'}`;
  };
  ...
  const CardContent = () => (
    <View className="relative p-6 bg-background border border-secondary rounded-2xl">
      {/* Badge populaire */}
      {isPopular && (
  ...
  // Si c'est le plan populaire, wrapper avec le gradient
  if (isPopular && !isCurrentPlan) {
    return (
      <View className="relative">
        {/* Effet glow */}
        <View className="absolute inset-0 bg-secondary opacity-20 blur-xl rounded-2xl transform scale-105" />
        <CardContent />
      </View>
    );
  }
